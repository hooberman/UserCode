...
void doTemplatePrediction( bool printgif = false ){
  ...
  */
  ...
  //TFile *f = TFile::Open(Form("output/%s/babylooper_lepdata_skim_EGStitchedTemplate_vecjetptreweight.root",iter));
  TFile *f = TFile::Open(Form("../output/%s/babylooper_pr_data_EGStitchedTemplate.root",iter));
  ...
  mcfilenames.push_back(Form("../output/%s/babylooper_zjets_PhotonJetTemplate.root",iter));     mcleg.push_back("DY");
  ...
  for( unsigned int i = 0 ; i < nhist ; i++ ){
    ...
    if(drawpull)    mainpad[i] = new TPad(Form("mainpad_%i",i),Form("mainpad_%i",i),0,0,1,0.75);
    ...
    metObserved[i]->GetXaxis()->SetTitle( "E_{T}^{miss} (GeV)" );
    metObserved[i]->GetYaxis()->SetTitle( "Events / 5 GeV" );
    metObserved[i]->GetYaxis()->SetTitleOffset(1);
    metObserved[i]->SetTitle( Form("%s %s",sample.c_str(),title[i].c_str()) );
    metPredicted[i]->SetLineColor(4);
    metPredicted[i]->SetLineWidth(3);
    ...
    cout << delimstart << setw(width1) << ""                << setw(width2)
         << delim      << setw(width1) << Form("N(met>%.0f GeV)",metval1)  << setw(width2)
         << delim      << setw(width1) << Form("N(met>%.0f GeV)",metval2)  << setw(width2) 
         << delim      << setw(width1) << Form("N(met>%.0f GeV)",metval3)  << setw(width2) << delimend << endl;
    ...
    cout << delimstart << setw(width1) << em                                    << setw(width2)
    ...
    printLine();
    cout << delimstart << setw(width1) << Form("Z pred + %s",em)                                          << setw(width2)
    ...
    TLegend *leg = new TLegend(0.7,0.45,0.95,0.9);
    leg->AddEntry(metObserved[i],"Observed","p");  
    if(addttbar){
      leg->AddEntry(metPredicted[i],"Z pred + e#mu");
      leg->AddEntry(httmc_clone,    "e#mu","l");
    }else{
      leg->AddEntry(metPredicted[i],"Z pred");
    }
    for(unsigned int iMC = 0 ; iMC < nMC ; iMC++ ){
      //leg->AddEntry(metObserved_MC[i][iMC],mcleg[iMC],"f");
      leg->AddEntry(metObserved_MC[i][nMC - iMC - 1],mcleg[nMC - iMC - 1],"f");
    }
    leg->SetFillColor(0);
    leg->SetBorderSize(1);
    ...
    TLatex *t = new TLatex();
    t->SetNDC();
    t->DrawLatex(0.4,0.87, "CMS");
    t->DrawLatex(0.4,0.82,  "23 pb^{-1}  at  #sqrt{s} = 7 TeV");

    if( TString(predhist[i]).Contains("ee") )
      t->DrawLatex(0.4,0.77, "Events with ee");
    else if( TString(predhist[i]).Contains("mm") )
      t->DrawLatex(0.4,0.77, "Events with #mu#mu");
    else
      t->DrawLatex(0.4,0.77, "Events with ee/#mu#mu");


    if( drawpull ){

      can[i]  -> cd();
      pullpad[i] = new TPad(Form("pullpad_%i",i),Form("pullpad_%i",i),0,0.75,1,1.);
      pullpad[i] -> Draw();
      pullpad[i] -> cd();
...
TH1F* getPullHist(TH1F* h1, TH1F* h2){
  ...
  for(int ibin = 1 ; ibin <= h1->GetNbinsX() ; ibin++){
    ...
    //if( h1->GetBinContent(ibin) > 0 && h2->GetBinContent(ibin) > 10 ){
