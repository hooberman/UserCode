...
void Z_looper::ScanChain (TChain* chain, const char* prefix, bool isData,
  ...
  if( TString(prefix).Contains("sms") || TString(prefix).Contains("T5") ){
    ...
    set_vtxreweight_rootfile("vtxreweight_Summer12MC_PUS7_920pb_Zselection.root",true);
  }
  ...
  while ((currentFile = (TFile*)fileIter.Next())){
	...
	if     ( TString(evt_dataset().at(0)).Contains("TuneZ2_s-")  ) st_ = 0;
	  ...
	  if( ! passElectronSelection_ZMet2012_v1(iel) ) continue;
	...
	if( debug ){
	  ...
	  if( abs(hyp_ll_id()[hypIdx]) == 11 )   cout << "ele ll  " << passElectronSelection_ZMet2012_v1(hyp_ll_index()[hypIdx]) << endl;
