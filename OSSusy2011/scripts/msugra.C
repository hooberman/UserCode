...
float getUpperLimit( float seff ){
  ...
  if( seff > 0.5 ) cout << "Error, signal efficiency uncertainty too large! " << seff << endl;
  return ul;
}
...
void msugra( char* filename ){
 ...
 TH2F* hyield_k   = (TH2F*) f->Get("msugra_highmet");
 TH2F* hyield_kup = (TH2F*) f->Get("msugra_highmet_kup");
 TH2F* hyield_kdn = (TH2F*) f->Get("msugra_highmet_kdn");
 TH2F* hyield_jup = (TH2F*) f->Get("msugra_highmet_jup");
 TH2F* hyield_jdn = (TH2F*) f->Get("msugra_highmet_jdn");
