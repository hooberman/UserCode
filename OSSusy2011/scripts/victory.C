...
const int   width1      = 20;
const int   width2      = 4;
const char* iter        = "../output/V00-00-05/highpt/";
...
float getMCratio( TH1F* hobs , TH1F* hpred , float cut );
...
void victory( bool printgif =false ){
  ...
  char* jetcutstring = "htpf>125&&htpf<300&&npfjets>1";
  if( issignal ) jetcutstring = "htpf>300&&npfjets>1";
  ...
  */
  ...
  title->DrawLatex(0.58,0.15,"L_{int} = 90 pb^{-1},  #sqrt{s} = 7 TeV");
  ...
  float mcratio = getMCratio(  hmety_mctot , hdilpty_mctot , 8.5 );  

  if( plotData ){
    printRow( "data" , hmety_data , hdilpty_data , 8.5 , mcratio );
    printLine();
  }
...
void plotHist( TH1F* hpred , TH1F* hobs , TH1F* hpred_data, TH1F* hobs_data,
  ...
  line.SetLineWidth(2);
  line.SetLineStyle(2);
  line.DrawLine( metcut , 0.001 , metcut , 1000 );
