...
void tnpScale_IDISO_mu( bool printplot = false ) {
  ...
  for( int i = 0 ; i < 5 ; i++ ){
    ...
    if( printplot ) c_iso[i]->Print(Form("plots/iso_mu_njets%i.pdf",i));
    ...
    if( printplot ) c_id[i]->Print(Form("plots/id_mu_njets%i.pdf",i));
