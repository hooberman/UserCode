...
// Class:      PixTcMet
// 
/**\class PixTcMet PixTcMet.cc Module/PixTcMet/src/PixTcMet.cc
...
*/
//
// Original Author:  Benjamin Henry Hooberman,32 1-B12,+41227670718,
//         Created:  Fri Dec 10 13:00:51 CET 2010
// $Id$
//
//
...
class PixTcMet : public edm::EDProducer {
public:
  explicit PixTcMet(const edm::ParameterSet&);
  ~PixTcMet();
...
private:
  ...
  TH2D* response_function;

  void correctSumEtForTrack( const reco::TrackRef track  , TH2D* rf , TVector3 outerTrackPosition );
  void correctMETforTrack( const reco::TrackRef track , TH2D* rf , TVector3 outerTrackPosition );
  bool isGoodTrack( const reco::TrackRef track , int trk_idx);
  TVector3 propagateTrack( const reco::TrackRef track );
...
//
PixTcMet::PixTcMet(const edm::ParameterSet& iConfig):
...
void
PixTcMet::produce(edm::Event& event, const edm::EventSetup& iSetup)
{
   using namespace edm;
   ...
   for( unsigned int trk_idx = 0; trk_idx < nTracks; trk_idx++ ) {
     ...
     const TVector3 outerTrackPosition = propagateTrack( trkref );  
...
bool PixTcMet::isGoodTrack( const reco::TrackRef track , int trk_idx) {
...
void PixTcMet::correctMETforTrack( const reco::TrackRef track , TH2D* rf , TVector3 outerTrackPosition ) {
...
void PixTcMet::correctSumEtForTrack( const reco::TrackRef track  , TH2D* rf , TVector3 outerTrackPosition ) {
...
TVector3 PixTcMet::propagateTrack( const reco::TrackRef track ) {
...
TH2D* PixTcMet::getResponseFunction_fit() {
