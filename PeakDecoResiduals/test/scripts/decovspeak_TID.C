...
void decovspeak(bool printgif = false){
  ...
  for(unsigned int ifile = 0 ; ifile < nfiles ; ifile++){
    ...
    hdu[ifile] = (TH1F*) file[ifile]->Get("PeakDecoResiduals/PeakDecoResiduals/du_TID");
    hdw[ifile] = (TH1F*) file[ifile]->Get("PeakDecoResiduals/PeakDecoResiduals/dw_TID");

    //    if( strcmp(types.at(ifile),"deco (BP)") == 0){
      hduthp[ifile] = (TH2F*) file[ifile]->Get("PeakDecoResiduals/PeakDecoResiduals/duvsdtantheta_TID_vp_wp_all");
      TH2F* htp     = (TH2F*) file[ifile]->Get("PeakDecoResiduals/PeakDecoResiduals/duvsdtantheta_TID_vp_wm_all");
      hduthp[ifile] -> Add(htp);
      
      hduthm[ifile] = (TH2F*) file[ifile]->Get("PeakDecoResiduals/PeakDecoResiduals/duvsdtantheta_TID_vm_wp_all");
      TH2F* htm     = (TH2F*) file[ifile]->Get("PeakDecoResiduals/PeakDecoResiduals/duvsdtantheta_TID_vm_wm_all");
      hduthm[ifile] -> Add(htm);
    // }else{
//       hduthp[ifile] = (TH2F*) file[ifile]->Get("PeakDecoResiduals/PeakDecoResiduals/duvsdtantheta_TID_vp_wp");
//       TH2F* htp     = (TH2F*) file[ifile]->Get("PeakDecoResiduals/PeakDecoResiduals/duvsdtantheta_TID_vp_wm");
//       hduthp[ifile] -> Add(htp);
      
//       hduthm[ifile] = (TH2F*) file[ifile]->Get("PeakDecoResiduals/PeakDecoResiduals/duvsdtantheta_TID_vm_wp");
//       TH2F* htm     = (TH2F*) file[ifile]->Get("PeakDecoResiduals/PeakDecoResiduals/duvsdtantheta_TID_vm_wm");
//       hduthm[ifile] -> Add(htm);
//     }
  ...
  if(draw("du_dw",cantitles)){
    ...
    plotHists(hdu,nfiles,"TID","#Delta u [#mum]",-1000,1000,10,3);
    leg1->Draw();

    can[idx]->cd(2);
    setStats(hdw,nfiles,0.8,0.65,0.15);  
    plotHists(hdw,nfiles,"TID","#Delta w [#mum]",-2000,2000,10,3);
    leg1->Draw();
  ...
  if(draw("duvsdth",cantitles)){
    ...
    for(unsigned int ifile = 0 ; ifile < nfiles ; ifile++){
      ...
      plotHistsTH2(hduthp[ifile],
                   Form("TID %s (v+)",types.at(ifile)),"tan(#theta_{trk})-tan(#theta_{L})",
  ...
  if(draw("duvsdth_tgraph",cantitles)){
    ...
    TGraphErrors *gduthpdiff = diffTGraph(gduthp[0],gduthp[2],"TID DECO - PEAK (v+)","<tan(#theta_{trk})-tan(#theta_{LA})>","<#Deltau> (#mum)");
    ...
    TGraphErrors *gduthmdiff = diffTGraph(gduthm[0],gduthm[2],"TID DECO - PEAK (v-)","<tan(#theta_{trk})-tan(#theta_{LA})>","<#Deltau> (#mum)");
  ...
  for(unsigned int ican=0;ican<ncan;ican++){
    can[ican]->Modified();
    can[ican]->Update();
    if(printgif) can[ican]->Print(Form("plots/%s_TID.gif",cantitles.at(ican)));
  }
