...
MyStrip->cd();
TH1F *dupeak=        cloneHist(h_Xprime_TIDBarrel_3);
TH1F *dwpeak=        cloneHist(h_resXprimeOverTheta_TIDBarrel_3);
TH1F *dttimepeak=    (TH1F*)h_dttime_TIDBarrel_3->Clone();
TH1F *dttimeerrpeak= (TH1F*)h_dttimeerr_TIDBarrel_3->Clone();
TH1F *ndtpeak=       (TH1F*)h_ndt_TIDBarrel_3->Clone();
TH1F *chargepeak=    (TH1F*)h_charge_TIDBarrel_3->Clone(); 
TH1F *nstripspeak=   (TH1F*)h_nstrips_TIDBarrel_3->Clone(); 

fdeco.cd();
TrackerOfflineValidation->cd();
MyStrip->cd();
TH1F *dudeco=        cloneHist(h_Xprime_TIDBarrel_3);
TH1F *dwdeco=        cloneHist(h_resXprimeOverTheta_TIDBarrel_3);
TH1F *dttimedeco=    (TH1F*)h_dttime_TIDBarrel_3->Clone();
TH1F *dttimeerrdeco= (TH1F*)h_dttimeerr_TIDBarrel_3->Clone();
TH1F *ndtdeco=       (TH1F*)h_ndt_TIDBarrel_3->Clone();
TH1F *chargedeco=    (TH1F*)h_charge_TIDBarrel_3->Clone(); 
TH1F *nstripsdeco=   (TH1F*)h_nstrips_TIDBarrel_3->Clone(); 

//draw 1D histo-----------------------------------------------
c1->cd(1);
setStats(dupeak,dudeco,0.5,0.65,0.15,false);  
plotHists(dupeak,dudeco,"TID","#Delta u [#mum]",-500,500,1,3);
leg1->Draw();
c1->cd(2);
setStats(dwpeak,dwdeco,0.5,0.65,0.15,false);  
plotHists(dwpeak,dwdeco,"TID","#Delta w [#mum]",-1000,1000,1,3);
leg1->Draw();

c4->cd(1);
setStats(dttimepeak,dttimedeco,0.5,0.65,0.15,false);  
plotHists(dttimepeak,dttimedeco,"TID","DT time (ns)",-50,50,1,0);
leg1->Draw();
c4->cd(2);
setStats(dttimeerrpeak,dttimeerrdeco,0.5,0.65,0.15,false);  
plotHists(dttimeerrpeak,dttimeerrdeco,"TID","#delta DT time (ns)",0,100,1,0);
...
plotHists(ndtpeak,ndtdeco,"TID","n DT Hits",0,50,1,0);
leg1->Draw();

c10->cd(1);
setStats(chargepeak,chargedeco,0.5,0.65,0.15,false);  
plotHists(chargepeak,chargedeco,"TID","Charge",0,1000,1,0);
c10->cd(2);
setStats(nstripspeak,nstripsdeco,0.5,0.65,0.15,false);  
plotHists(nstripspeak,nstripsdeco,"TID","NStrips",0,10,1,0);
...
TrackerOfflineValidation->cd();
MyStrip->cd();
TH2F* duthetapeak =  (TH2F*)h_resXprimeVsTheta_TIDBarrel_3->Clone();
//TH2F* dutrkpeak =  (TH2F*)h_duvstrkangle_TIDBarrel_3->Clone();
//TH2F* dulorpeak =  (TH2F*)h_duvslorangle_TIDBarrel_3->Clone();
TH2F* duvsdtpeak =   (TH2F*)h_duvsdttime_TIDBarrel_3->Clone();
TH2F* dwvsdtpeak =   (TH2F*)h_dwvsdttime_TIDBarrel_3->Clone();
TH2F* chvsdtpeak = (TH2F*)h_chargevsdttime_TIDBarrel_3->Clone();

fdecoTH2.cd();
TrackerOfflineValidation->cd();
MyStrip->cd();
TH2F* duthetadeco =  (TH2F*)h_resXprimeVsTheta_TIDBarrel_3->Clone();
//TH2F* dutrkdeco =  (TH2F*)h_duvstrkangle_TIDBarrel_3->Clone();
//TH2F* dulordeco =  (TH2F*)h_duvslorangle_TIDBarrel_3->Clone();
TH2F* duvsdtdeco =   (TH2F*)h_duvsdttime_TIDBarrel_3->Clone();
TH2F* dwvsdtdeco =   (TH2F*)h_dwvsdttime_TIDBarrel_3->Clone();
TH2F* chvsdtdeco = (TH2F*)h_chargevsdttime_TIDBarrel_3->Clone();

//draw 2D histo-----------------------------------------------
plotHistsTH2(duthetapeak,
	     "TID PEAK","tan(#theta_{trk})-tan(#theta_{L})",
	     "#Delta u [#mum]",-2,2,-200,200,0,c2,1);
plotHistsTH2(duthetadeco,
	     "TID DECO","tan(#theta_{trk})-tan(#theta_{L})",
...
string projtitle[nslices]={"TID (-20 ns < dt < -12 ns)",
			   "TID (-12 ns < dt < -8 ns)",
			   "TID (-8 ns < dt < -4 ns)",
			   "TID (-4 ns < dt < 0 ns)",
			   "TID (0 ns < dt < 4 ns)",
...
for(int islice=0;islice<nslices;islice++){
  ...
  //plotHists(dtpeakproj[islice],dtdecoproj[islice],"TID","DT Time [ns]",-20,20,1,0);
...
if(printgif){
  c1->Print (Form("%s/plots/deltau_deltaw_TID.gif",dir));
  c2->Print (Form("%s/plots/duvstantheta_TID.gif",dir));
  c3->Print (Form("%s/plots/duvsdt_TID.gif",dir));
  c4->Print (Form("%s/plots/dttime_TID.gif",dir));
  c5->Print (Form("%s/plots/dwslice_TID.gif",dir));
  c7->Print (Form("%s/plots/dwvsdt_TID.gif",dir));
  c10->Print(Form("%s/plots/charge_nstrips_TID.gif",dir));
  c11->Print(Form("%s/plots/chargevsdt_TID.gif",dir));
  c12->Print(Form("%s/plots/chargeslice_TID.gif",dir));
  c13->Print(Form("%s/plots/chargevsdtslice_TID.gif",dir));

  //c6->Print(Form("localjobs/%s/plots/dtslice_TID.gif",dir));
}
