// -*- C++ -*-
//
// Package:    PeakDecoResiduals4
// Class:      PeakDecoResiduals4
// 
/**\class PeakDecoResiduals4 PeakDecoResiduals4.cc Alignment/Validator/src/PeakDecoResiduals4.cc
...
// $Id: PeakDecoResiduals4.cc,v 1.12 2010/10/14 13:21:00 benhoob Exp $
//
//
...
#include "Alignment/PeakDecoResiduals4/interface/TrackerValidationVariables.h"
#include "Alignment/PeakDecoResiduals4/interface/TkOffTreeVariables.h"
...
class PeakDecoResiduals4 : public edm::EDAnalyzer {
public:
  explicit PeakDecoResiduals4(const edm::ParameterSet&);
  ~PeakDecoResiduals4();
...
//
template <class OBJECT_TYPE>  
int PeakDecoResiduals4::GetIndex(const std::vector<OBJECT_TYPE*> &vec, const TString &name)
{
  int result = 0;
  ...
  edm::LogError("Alignment") << "@SUB=PeakDecoResiduals4::GetIndex" << " could not find " << name;
  return -1;
}


// constructors and destructor
PeakDecoResiduals4::PeakDecoResiduals4(const edm::ParameterSet& iConfig)
...
PeakDecoResiduals4::~PeakDecoResiduals4()
{
  if(debug_) cout<<"PeakDecoResiduals4::~PeakDecoResiduals4"<<endl;
...
void PeakDecoResiduals4::BookHists(TFileDirectory &tfd){

  if(debug_) cout<<"PeakDecoResiduals4::bookHists"<<endl;
...
bool PeakDecoResiduals4::isBarrel(uint32_t subDetId)
{
...
bool PeakDecoResiduals4::isEndCap(uint32_t subDetId)
{
...
bool PeakDecoResiduals4::isPixel(uint32_t subDetId)
{
  return (subDetId == PixelSubdetector::PixelBarrel || subDetId == PixelSubdetector::PixelEndcap);
}


bool PeakDecoResiduals4::isDetOrDetUnit(align::StructureType type)
{
  return ( type == align::AlignableDet || type == align::AlignableDetUnit);
}


int PeakDecoResiduals4::getSubdetInteger(const DetId& detid){
  ...
  if     (subdetid  == PixelSubdetector::PixelBarrel)   return 0;
    ...
    throw cms::Exception("Geometry Error")
      << "[PeakDecoResiduals4] Error, tried to get reference for non-tracker subdet " << subdetid 
...
void
PeakDecoResiduals4::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{

  if(debug_) cout<<"PeakDecoResiduals4::analyze"<<endl;
  int evt_run                       = iEvent.id().run()        ;
  int evt_lumiBlock                 = iEvent.luminosityBlock() ;  
  int evt_event                     = iEvent.id().event()      ;

  //cout<<"PeakDecoResiduals4 Run: "<<evt_run<<" LumiBlock: "<<evt_lumiBlock<<endl;
  ...
  if(debug_) cout<<"End PeakDecoResiduals4::analyze"<<endl;

}

void PeakDecoResiduals4::makeHist( DetId detid , TFileDirectory &tfd ){
...
DEFINE_FWK_MODULE(PeakDecoResiduals4);
