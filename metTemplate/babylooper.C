...
void babylooper::ScanChain (TChain* chain, const char* iter, const char* prefix, bool isData, 
...
//     reweightHist[2]->Draw();
  ...
  if( !makeTemplate ){
    ...
    if( isData ){
      ...
      if( myTemplateSource == e_QCD ){
        ...
        templateFileName = Form("photonTemplates/nov5th_v4/photon_templates_default.root",iter);
        //templateFileName = Form("photonTemplates/nov5th_v4/photon_templates_hoe05.root",iter);
        //templateFileName = Form("photonTemplates/nov5th_v4/photon_templates_vecjetptreweight.root",iter);
        //templateFileName = Form("photonTemplates/nov5th_v4/photon_templates_emf09.root",iter);
        cout << "Using template file " << templateFileName << endl;
  ...
  ofstream eefile(Form("output/%s/%s_ee.txt",iter,prefix));
  ofstream mmfile(Form("output/%s/%s_mm.txt",iter,prefix));
  ...
  while ((currentFile = (TFile*)fileIter.Next())){
	...
	if( leptype_ == 0 && npfmuons_ < 1 ) continue;
