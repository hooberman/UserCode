...
void makeRootFile_OS_shape(){
  ...
  float   LM1_yield[nbins]            = {  71.1    ,   22.7   ,   71.3   ,   19.8   ,   42.9   ,   8.1  }
  float   LM1_yield_JESup[nbins]      = {  75.3    ,   23.7   ,   89.8   ,   23.2   ,   46.7   ,  10.3  }
  float   LM1_yield_JESdn[nbins]      = {  66.8    ,   21.9   ,   55.1   ,   15.0   ,   35.7   ,   7.3  }
  ...
  TH1F* histo_LM1               = new TH1F("histo_LM1","histo_LM1",nbins,0,nbins);
  TH1F* histo_LM1_JES_shapeUp   = new TH1F("histo_LM1_JES_shapeUp"  ,"histo_LM1_JES_shapeUp"  ,nbins,0,nbins);
  TH1F* histo_LM1_JES_shapeDown = new TH1F("histo_LM1_JES_shapeDown","histo_LM1_JES_shapeDown",nbins,0,nbins);
  ...
  for( unsigned int ibin = 0 ; ibin < nbins ; ibin++){
    histo_LM1                  -> SetBinContent(ibin+1,LM1_yield[ibin]);
    histo_LM1_JES_shapeUp      -> SetBinContent(ibin+1,LM1_yield_JESup[ibin]);
    histo_LM1_JES_shapeDown    -> SetBinContent(ibin+1,LM1_yield_JESdn[ibin]);
  ...
  histo_Data->Write();
  histo_LM1->Write();
  histo_LM1_JES_shapeUp->Write();
  histo_LM1_JES_shapeDown->Write();
  histo_LM6->Write();
